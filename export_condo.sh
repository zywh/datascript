...
#update city_id
echo "Update City_ID"
sql="update h_house set city_id=3 where county=\"Ontario\";"
`$sqlcmd -e "$sql"`

echo "Update property_type"
sql="
update h_house set propertyType_id=1 where type_own1_out='Detached';
update h_house set propertyType_id=2 where type_own1_out in ('Townhouse' ,'Att/Row/Twnhouse','Triplex','Fourplex','Multiplex');
update h_house set propertyType_id=4 where type_own1_out in ('Semi-Detached','Link','Duplex');
update h_house set propertyType_id=5 where type_own1_out in ('Cottage','Rural Resid');
...
update h_house set propertyType_id=3 where type_own1_out like 'Co%';
update h_house set propertytype_id=8 where propertytype_id='';"

`$sqlcmd -e "$sql"`

echo "Update district_id"
sql="
update h_house h join h_district d on h.area=d.englishName set h.district_id = d.id;
"
...
#Fix yr_built
sql=" update h_house set community = replace(community,\"'\",\"-\");
update h_house set yr_built=\"6-15\" where yr_built=\"6-10\";
update h_house set yr_built=\"6-15\" where yr_built=\"11-15\";
...
sql="
update h_stats set avg_price = (select avg(lp_dol) from h_house where s_r='Sale' and lp_dol > 1000) where date=CURRENT_DATE( ); 
"
`$sqlcmd -e "$sql"`
...
;
"
echo "Load CREA Export into Maplecity DB"
`$sqlcmd -e "$sql"`
